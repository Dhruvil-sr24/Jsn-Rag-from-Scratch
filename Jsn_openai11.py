...
class ChatRequest(BaseModel):
    question: str
    session_id: str = "default"  # Default session like your working version

class ChatResponse(BaseModel):
    question: str
    answer: str
    session_id: str  # Always return session_id
    conversation_stage: str
...
# ADD or REPLACE with these three definitive functions.
    ...
    def generate_qualification_question(self, question: str, context: ConversationContext) -> str:
        ...
        **Your Task:**
        If user greet you then you should start asking question with greeting.
        Generate ONE specific, clarifying question that will help you understand:
        1. What specific solution they need (if unclear)
        2. Any missing requirements or constraints
        3. Technical preferences or business goals
        4. If business type is not known THEN ASK FOR business type.
    ...
    def _extract_business_with_llm(self, question: str, conversation_history: str, context: ConversationContext) -> bool:
        ...
        **CRITICAL INSTRUCTIONS:**
        1. If business or industry type is already known, DO NOT change it unless user explicitly corrects it
        2. If needs already exist, ADD to them, don't replace unless user explicitly corrects
        3. Look for industry keywords: ecommerce, restaurant, healthcare, automotive, IT, etc.
    ...
    def detect_topic_change_with_llm(self, question: str, context: ConversationContext) -> Dict[str, Any]:
        ...
        **Examples:**
        ...
        try:
            ...
            result=self._parse_topic_analysis(analysis_text)
            if not isinstance(result, dict):
                logger.error(f"Topic analysis returned non-dict: {type(result)}")
                return {
                    "topic_changed": False,
                    "new_topic": None,
                    "reasoning": "Analysis failed - fallback"
                }
...
class AdvancedRAGChatbot:
    ...
    def chat(self, question: str) -> Dict[str, Any]:
        """
        FIXED: Main chat method with proper error handling
        """
        ...
        try:
            ...
            if len(self.messages) > 1:
                ...
                # FIXED: Ensure topic_analysis is always a dict
                if not isinstance(topic_analysis, dict):
                    logger.error(f"Topic analysis returned {type(topic_analysis)}, expected dict")
                    topic_analysis = {
                        "topic_changed": False,
                        "new_topic": None,
                        "reasoning": "Invalid analysis result"
                    }
                
                if topic_analysis.get("topic_changed", False):
            ...
            if self.context.pending_confirmation:
                last_bot_question = self.messages[-2].get("content", "") if len(self.messages) >= 2 else ""
            ...
            # FIXED: Ensure response_data is always a dict
            if not isinstance(response_data, dict):
                logger.error(f"Response data is {type(response_data)}, expected dict")
                response_data = {
                    "question": question,
    ...
    def _format_qualification_response(self, question: str, response: str) -> Dict[str, Any]:
        """FIXED: Formats a standard qualification response dictionary safely."""
        return {
            "question": question,
    ...
    def _format_error_response(self, question: str) -> Dict[str, Any]:
        """FIXED: Formats a standard error response dictionary."""
        return {
            "question": question,
...
class FrontendSessionManager:
    """Simple session manager that uses frontend-generated session IDs"""
    
    def __init__(self):
        self.sessions: Dict[str, AdvancedRAGChatbot] = {}
        self.session_metadata: Dict[str, Dict] = {}
        logger.info("📁 Frontend Session Manager initialized")
    
    def get_or_create_chatbot(self, session_id: str) -> tuple[AdvancedRAGChatbot, bool]:
        """
        Get or create chatbot for the given session_id (generated by frontend)
        Returns (chatbot, is_new_session)
        """
        is_new_session = session_id not in self.sessions
        
        if is_new_session:
            logger.info(f"✨ Creating new session for frontend ID: {session_id}")
            
            # Create chatbot instance
            chatbot = AdvancedRAGChatbot(cache_dir=f"./cache/{session_id}")
            self._initialize_chatbot(chatbot)
            
            # Store chatbot and metadata
    ...
    def get_session_info(self, session_id: str) -> Dict:
        ...
        return {
            ...
            'conversation_stage': chatbot.context.conversation_stage,
            'business_context': chatbot.context.business_context,
    ...
    def get_all_sessions(self) -> Dict:
        ...
        return {
            ...
            'metadata': {sid: self.session_metadata[sid] for sid in self.sessions.keys()}
        }
    
    def _initialize_chatbot(self, chatbot: AdvancedRAGChatbot):
        """Initialize chatbot with resources"""
        try:
...
async def lifespan(app: FastAPI):
    ...
    session_manager = FrontendSessionManager()
    logger.info("🎉 Frontend Session Manager initialized!")
    yield
    logger.info("🛑 Session management shutdown")

app = FastAPI(
    title="RAG Chatbot with Frontend Session Management",
    version="1.0.0",
    lifespan=lifespan
)
...
async def chat_endpoint(request: ChatRequest):
    """SAFE chat endpoint with proper error handling"""
    global session_manager
    ...
    try:
        # Get or create chatbot - with safety checks
        if hasattr(session_manager, 'get_or_create_chatbot'):
            result = session_manager.get_or_create_chatbot(request.session_id)
            if isinstance(result, tuple) and len(result) == 2:
                chatbot, is_new_session = result
            else:
                raise ValueError(f"Invalid session manager response: {type(result)}")
        else:
            raise ValueError("Session manager missing get_or_create_chatbot method")
        
        # Validate chatbot
        if not hasattr(chatbot, 'chat'):
            raise ValueError(f"Invalid chatbot object: {type(chatbot)}")
        
        # Process chat with safety
        response_data = chatbot.chat(request.question)
        
        # Validate response
        if not isinstance(response_data, dict):
            raise ValueError(f"Chat returned invalid type: {type(response_data)}")
        
        # Ensure all required fields exist
        required_fields = ['question', 'answer', 'conversation_stage', 'confidence_level']
        for field in required_fields:
            if field not in response_data:
                response_data[field] = "unknown" if field != 'confidence_level' else 0.0
        
        # Update session
        session_manager.increment_message_count(request.session_id)
        session_info_dict = session_manager.get_session_info(request.session_id)
        
        # Create response
        session_info = SessionInfo(
            session_id=request.session_id,
            created_at=session_info_dict.get('created_at', datetime.now().isoformat()),
            last_activity=datetime.now().isoformat(),
            messages_count=session_info_dict.get('message_count', 0),
            conversation_stage=response_data['conversation_stage'],
            business_context=session_info_dict.get('business_context'),
...
async def delete_session_endpoint(request: dict):
...
@app.get("/")
async def root():
    return {"message": "RAG Chatbot with Frontend Session Management is running!", "version": "1.0.0"}
